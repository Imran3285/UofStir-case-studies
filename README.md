# 👋 Hi, I'm Muhammad Imran

🎓 MSc Data Science for Business | ⚡ Aspiring Data Scientist in Healthcare & AI |
📍 Based in Scotland | Building data-driven and AI-powered solutions for healthcare innovation and smarter decision-making

---

## 🚀 Portfolio Overview

This repository showcases a collection of my academic and consulting-style projects, where I apply data analytics, machine learning concepts, and business strategy to solve real-world problems across domains such as healthcare, transport, retail, and sustainability.

Through these projects, I demonstrate my ability to:

* Analyse complex datasets and extract meaningful insights  
* Apply statistical and analytical techniques to real-world scenarios  
* Deliver data-driven recommendations for business improvement  
* Bridge the gap between technical analysis and strategic decision-making  

---

## 📂 Projects

### 🏥 AI Implementation in NHS: Overcoming Challenges for a Smart Healthcare System

**Description:**  
Business consulting case study exploring the implementation of AI in the NHS to enhance efficiency, reduce waiting times, and enable smarter healthcare systems.

**Key Highlights:**

* Analysed key operational challenges within the public healthcare system  
* Proposed AI-driven solutions to improve productivity and service delivery  
* Focused on reducing administrative workload and enhancing patient outcomes  

---

### 🚲 Leveraging Data Science to Support Green Transitions — Independent Project

**Description:**  
Data science and consulting project analysing the impact of electrification, bike-sharing schemes, and Low Emission Zones (LEZs) in supporting sustainable transport and reducing carbon emissions in Scotland.

**Key Highlights:**

* Applied time series, geospatial, and comparative analysis to real-world transport datasets  
* Evaluated the effectiveness of bike-sharing, EV infrastructure, and LEZ policies  
* Used Power BI for interactive dashboards and spatial analysis  
* Implemented CRISP-DM methodology for structured data analysis  
* Delivered data-driven recommendations to support net-zero goals by 2045  

---

### 🛍️ JCPenney Retail Data Analysis: Customer Behaviour & Pricing Insights 

**Description:**  
Python-based data analytics project analysing retail datasets to generate actionable business insights and support decision-making.

**Key Highlights:**

* Performed data cleaning and preprocessing using Pandas and NumPy  
* Conducted exploratory data analysis and visualisation using Matplotlib  
* Applied sentiment analysis to understand customer feedback  
* Identified key patterns in customer behaviour and pricing strategies  

---

### 🌱 Solasta Bio's Innovation Journey: Opportunities and Challenges in Biopesticides

**Description:**  
Innovation management case study evaluating Solasta Bio’s business model, market opportunities, and growth challenges.

**Key Highlights:**

* Applied Doblin’s Ten Types of Innovation framework  
* Analysed industry trends and competitive landscape  
* Proposed strategic recommendations to support growth in the biopesticides market  

---

### 📈 Personal Development Plan

**Description:**  
Personal and professional development portfolio outlining my journey toward becoming a Data Scientist and AI professional, with a strong focus on healthcare analytics and data-driven innovation in Ireland. The project highlights my career goals, technical skill development, and a structured roadmap for continuous learning and professional growth.

**Key Highlights:**

* Conducted SWOT analysis and professional skills assessment
* Defined clear short-, medium-, and long-term career goals
* Developed a structured learning and career progression plan
* Focused on building expertise in Python, SQL, Power BI, and Machine Learning
* Identified strategies for continuous technical and professional development
* Aligned career ambitions with opportunities in healthcare and AI-driven industries

---

## 🧠 Skills Demonstrated

* **Programming:** Python, SQL, R  
* **Data Analysis:** Pandas, NumPy, Excel, Power BI  
* **Statistics:** Hypothesis testing, data interpretation  
* **Visualisation:** Matplotlib, Tableau, BI tools  
* **Business & Strategy:** Consulting analysis, innovation frameworks  
* **Tools:** GitHub, Anaconda, Azure, Simul8  

---

## 🎯 Career Focus

I am actively seeking opportunities in:

* Data Analyst  
* Data Scientist  
* AI / Machine Learning roles  
* Business Analytics / Consulting  

I am particularly interested in applying data-driven solutions within **healthcare, the public sector, and technology-driven organisations**, where analytics can create meaningful real-world impact.

---

## 📬 Let's Connect

Feel free to explore my projects and connect with me for collaboration or opportunities.

📧 Email: m.imran88222@gmail.com  
💻 GitHub: https://github.com/Imran3285  
🔗 LinkedIn: https://www.linkedin.com/in/imran00286  
